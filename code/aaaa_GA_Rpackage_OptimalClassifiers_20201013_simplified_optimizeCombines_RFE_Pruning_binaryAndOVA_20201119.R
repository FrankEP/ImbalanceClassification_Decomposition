...
#  subsets_rfe <- subsets_rfe * 2
  ...
  for(bootstrap_i in 1:num_bootstrap){    ## Stack1 Way1: normal for loop
    ...
    auc_model_list_bootstrap <- foreach(combination_i = c(1:ncol(class_combinations))) %DO% {       ### Stack2 Way2: dopar loop
      ...
      results_Wilcoxon <- cbind(names_x, results_Wilcoxon)
          ...
          for (row_i in 1:nrow(parameters_grid)) {
            ...
            if(rfe_select){
              ...
              for (subset_i in seq_len(length(subsets_backward))) {
                ...
                prediction_temp_temp <- prediction_temp_temp[, 2]
              ...
              names_x_select <- results_Wilcoxon$names_x[1:subsets_backward[which.max(auc_backward)]]
              ...
              for (subset_i in seq_len(length(subsets_backward))) {
                ...
                roc_OVA1_temp_temp <- pROC::roc(data_train_y_x_outbag_OVA1$Y_binary_factor, prediction_OVA1_temp_temp)
                threshold_OVA1_temp <- roc_OVA1_temp_temp$thresholds[which.max(roc_OVA1_temp_temp$sensitivities + roc_OVA1_temp_temp$specificities)]
                # auc_calculator(class_binary=c(class_1, class_2), label_All_classes=data_train_y_x_outbag$Y, prediction_All_classes=prediction_OVA1_temp_temp)
              ...
              for (subset_i in seq_len(length(subsets_backward))) {
                ...
                roc_OVA2_temp_temp <- pROC::roc(data_train_y_x_outbag_OVA2$Y_binary_factor, prediction_OVA2_temp_temp)
                threshold_OVA2_temp <- roc_OVA2_temp_temp$thresholds[which.max(roc_OVA2_temp_temp$sensitivities + roc_OVA2_temp_temp$specificities)]
            ...
            model_fit_list_temp[[row_i]] <- list(model_fit=model_fit, model_fit_OVA1=model_fit_OVA1, model_fit_OVA2=model_fit_OVA2)
...
for (bootstrap_i in 1:num_bootstrap) {
  ...
  for (combination_i in seq_len(length(idx_combinations[[bootstrap_i]]))) {
    ...
    model_storage_list[[bootstrap_i]][[idx_best_combination]] <- list(model_fit=model_fit, model_fit_OVA1=model_fit_OVA1, model_fit_OVA2=model_fit_OVA2)
    ...
    if(is.element(model_names[idx_best_model], c("RandomForest"))){
      ...
      prediction_inbag <- prediction_inbag[, 2]
...
for (bootstrap_i in 1:num_bootstrap) {
  ...
  for (combination_i in seq_len(length(idx_combinations[[bootstrap_i]]))) {
    ...
    if(is.element(model_names[idx_best_model], c("RandomForest"))){
      # all classes inbag
      ### binary
      prediction_inbag <- predict(model_fit, data = data_train_y_x)$predictions
      prediction_inbag <- prediction_inbag[, 2]
      ratio_inbag_temp <- 1/(max(prediction_inbag) - min(prediction_inbag))
      prediction_inbag <- (prediction_inbag - min(prediction_inbag)) * ratio_inbag_temp
      prediction_inbag <- -1 + prediction_inbag*2
      ...
      # all classes outbag
      ### binary
      prediction_outbag <- predict(model_fit, data = data_train_y_x)$predictions
      prediction_outbag <- prediction_outbag[, 2]
      ratio_outbag_temp <- 1/(max(prediction_outbag) - min(prediction_outbag))
      prediction_outbag <- (prediction_outbag - min(prediction_outbag)) * ratio_outbag_temp
      prediction_outbag <- -1 + prediction_outbag*2
