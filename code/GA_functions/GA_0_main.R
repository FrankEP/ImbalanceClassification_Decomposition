...
#prediction_matrix_inbag, N_row_inbag, label_real_inbag, num_combination, prediction_matrix_outbag, N_row_outbag, label_real_outbag,
               ...
               functionTest=FALSE) 
{
  ...
  if(is.character(fitness)) 
    { get(fitness, envir = .GlobalEnv) }
...
#    { stop("A fitness function must be provided") }
  ...
  callArgs <- list(...)
  if(functionTest) return(callArgs)
  ...
  Fitness_list <- rep(list(NA), popSize)
  ...
  for(iter in seq_len(maxiter))
     {
      ...
      if(!parallel)
        ...
        { Fitness_list <- foreach(j = seq_len(popSize), .combine="list", .multicombine = TRUE, .maxcombine = popSize) %DO%
                           { if(is.na(Fitness[j])){
                              tryCatch(do.call(fitness, c(list(c(Pop[j,])), callArgs)), error=function(err) NA) 
                             } 
                             else {
                               Fitness_list[[j]]
                             }
          ...
          Fitness <- foreach(k = seq_len(popSize), .combine = "c") %DO%
                      { base::ifelse(is.na(Fitness[k]), base::ifelse(is.na(Fitness_list[[k]]), NA, Fitness_list[[k]]$F1_outbag), Fitness[k])
                      }
        }
...
setClass("ga", slots = c(call="language", type="character", lower="numericOrNA", upper="numericOrNA", nBits="numericOrNA",
                             names="character", popSize="numeric", iter="numeric", run="numeric", maxiter="numeric", suggestions="matrix",
                             population="matrix", elitism="numeric", pcrossover="numeric", pmutation="numericOrNA", optim="logical",
                             fitness="numericOrNA", summary="matrix", bestSol="list", fitnessValue="numeric", solution="matrix"))
