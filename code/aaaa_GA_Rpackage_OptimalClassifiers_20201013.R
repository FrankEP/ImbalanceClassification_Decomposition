...
install.packages("clusterCrit", repos="https://mirrors.tuna.tsinghua.edu.cn/CRAN")
  ...
  for (combination_i in c(1:ncol(class_combinations))) {
    ...
    for (model_i in model_names) {
      ...
      for(parameter_i in seq_len(length(prediction_AllClass_outbag_list_temp))){
        ...
        cluster_criteria_temp <- clusterCrit::intCriteria(as.matrix(prediction_AllClass_inbag_temp), as.integer(data_train_y_x_inbag$Y), crit = "Davies_Bouldin")$davies_bouldin
        ...
        cluster_criteria_temp <- clusterCrit::intCriteria(as.matrix(prediction_AllClass_outbag_temp), as.integer(data_train_y_x_outbag$Y), crit = "Davies_Bouldin")$davies_bouldin
...
for (bootstrap_i in 1:num_bootstrap) {
  ...
  idx_combinations <- c()
  idx_models <- c()
  for(iter_i in seq_len(ncol(class_combinations))){
    cat("iter_i ", iter_i, "\n")
    
    if(iter_i==1){
      combinations_remain <- seq_len(ncol(class_combinations))
    } 
    
    cluster_criteria_models_combination <- c()
    for (combination_i in c(combinations_remain)) {
      cat("combination_i ", combination_i, "\n")
      
      idx_best_parameter_model <- idx_best_parameter_model_combination[, combination_i]
      clusterCtriteria_best_parameter_model <- clusterCtriteria_best_parameter_model_combination[, combination_i]
      auc_best_parameter_model <- auc_best_parameter_model_combination[, combination_i]
      prediction_AllClass_inbag_best_parameter_model <- prediction_AllClass_inbag_best_parameter_model_combination[[combination_i]]
      prediction_AllClass_outbag_best_parameter_model <- prediction_AllClass_outbag_best_parameter_model_combination[[combination_i]]
      
      cluster_criteria_inbag_models <- c()
      cluster_criteria_outbag_models <- c()
      for (model_i in model_names) {
        
        prediction_AllClass_inbag_best_parameter <- prediction_AllClass_inbag_best_parameter_model[, match(model_i, model_names)]
        prediction_AllClass_outbag_best_parameter <- prediction_AllClass_outbag_best_parameter_model[, match(model_i, model_names)]
        
        if(iter_i==1){
          prediction_AllClass_inbag <- prediction_AllClass_inbag_best_parameter
          prediction_AllClass_outbag <- prediction_AllClass_outbag_best_parameter
        } else {
          prediction_AllClass_inbag <- cbind(prediction_AllClass_inbag, prediction_AllClass_inbag_best_parameter)
          prediction_AllClass_outbag <- cbind(prediction_AllClass_outbag, prediction_AllClass_outbag_best_parameter)
        }
        
        cluster_criteria_inbag_temp <- clusterCrit::intCriteria(as.matrix(prediction_AllClass_inbag), as.integer(data_train_y_x_inbag$Y), crit = "Davies_Bouldin")$davies_bouldin
        cluster_criteria_inbag_models <- c(cluster_criteria_inbag_models, cluster_criteria_inbag_temp)
        
        cluster_criteria_outbag_temp <- clusterCrit::intCriteria(as.matrix(prediction_AllClass_outbag), as.integer(data_train_y_x_outbag$Y), crit = "Davies_Bouldin")$davies_bouldin
        cluster_criteria_outbag_models <- c(cluster_criteria_outbag_models, cluster_criteria_outbag_temp)
        
      }
      cluster_criteria_inbag_models <- ifelse(cluster_criteria_inbag_models==0, 0.001, cluster_criteria_inbag_models)
      cluster_criteria_outbag_models <- ifelse(cluster_criteria_outbag_models==0, 0.001, cluster_criteria_outbag_models)
      cluster_criteria_models_temp <- cbind(cluster_criteria_inbag_models/cluster_criteria_outbag_models, cluster_criteria_outbag_models/cluster_criteria_inbag_models)
      cluster_criteria_models_temp <- apply(cluster_criteria_models_temp, 1, max)*(cluster_criteria_inbag_models+cluster_criteria_outbag_models)
      cluster_criteria_models <- cluster_criteria_models_temp
      cluster_criteria_models[auc_best_parameter_model<0.9] <- NA
      # cluster_criteria_models <- cluster_criteria_inbag_models*cluster_criteria_outbag_models
      cluster_criteria_models_combination <- rbind(cluster_criteria_models_combination, cluster_criteria_models)
      
    }
    
    idx_model_i <- which(cluster_criteria_models_combination==min(cluster_criteria_models_combination, na.rm = "na.omit"), arr.ind = TRUE)[2]
    idx_combination_i <- combinations_remain[which(cluster_criteria_models_combination==min(cluster_criteria_models_combination, na.rm = "na.omit"), arr.ind = TRUE)[1]]
    
    idx_combinations <- c(idx_combinations, idx_combination_i)
    idx_models <- c(idx_models, idx_model_i)
    
    combinations_remain <- combinations_remain[!is.element(combinations_remain, idx_combination_i)]
    
    cat("combinations_remain ", combinations_remain, "\n")
...
GA_results <- ga(type="real-valued", fitness=fitness_function,
                 ...
                 keepBest = TRUE, elitism=floor(0.5*50),
                 suggestions = x_initial,
                 maxiter=1, run=1, maxFitness=1, parallel = num_cores)
