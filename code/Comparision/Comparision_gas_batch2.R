...
print("3")
source(paste0(path,'/code/aaaa_GA_Rpackage_OptimalClassifiers_20201013_simplified_optimizeCombines.R'))
...
data_all <- openxlsx::read.xlsx(paste0(path,"/data_open/", pathname,"/gas_batch2.xlsx"))

for (col_i in 2:ncol(data_all)) {
  data_all[, col_i] <- sub(".*[0-9]+_", "", data_all[, col_i])
  data_all[, col_i] <- as.numeric(data_all[, col_i])
}

name_y_data_all <- "Class"
names_x_data_all <- colnames(data_all)[c(2:129)]

data_all_y <- data_all[, name_y_data_all]
data_all_x <- data_all[, names_x_data_all]
data_all_x <- as.matrix(data_all_x)

data_all_x <- scale(data_all_x, scale = TRUE, center = FALSE)
...
for(rep_i in 2:num_rep){
  ...
  for(fold_i in 1:num_Kfold){
    ...
    if(is.element("ATM", names_methods)){
      results_ATM <- Adaptive_threshold_moving(X=data_train_x, Y=data_train_y, NEW_X=data_test_x, num_bootstrap=1, pathname = pathname, fold_i=fold_i, rep_i=rep_i, plot_scatter=FALSE)
