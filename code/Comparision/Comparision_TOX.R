...
data_all <- openxlsx::read.xlsx(paste0(path,"/data_open/", pathname,"/TOX_171.xlsx"), colNames = FALSE)

TOXtraidx <- c(167,14,51,100,46,106,41,116,65,136,27,32,153,30,29,86,44,57,140,159,
               1,47,58,120,119,166,37,134,105,111,72,17,12,84,141,109,104,40,75,16,
               114,15,26,77,71,158,23,147,60,68,135,34,8,90,59,13,157,103,49,139,
               92,127,39,118,156,25,81,33,125,87,170,115,171,88,63,160,45,122,102,113,
               91,146,165,24,149,98,56,144,80,121,4,83,163,70,108,126,101,62,93,152,
               38,9,107,132,74,99,61,54,143,76,7,112,130,168,138,162,18,123,79,22,
               11,31,55,67,131,169,145,10,97,21,52,2,161,73,69,117,28);
TOXtesidx <- c(48,36,78,53,95,148,151,89,110,155,124,133,66,19,3,42,154,137,128,85,
               164,35,6,142,43,96,5,50,20,82,94,64,129,150);
TOXtraclass <- c(4,1,2,3,2,3,1,3,2,4,1,1,4,1,1,2,1,2,4,4,1,2,2,3,3,4,1,4,3,3,2,1,1,2,4,3,3,1,2,1,
                 3,1,1,2,2,4,1,4,2,2,4,1,1,2,2,1,4,3,2,4,3,3,1,3,4,1,2,1,3,2,4,3,4,2,2,4,1,3,3,3,
                 3,4,4,1,4,3,2,4,2,3,1,2,4,2,3,3,3,2,3,4,1,1,3,4,2,3,2,2,4,2,1,3,4,4,4,4,1,3,2,1,
                 1,1,2,2,4,4,4,1,3,1,2,1,4,2,2,3,1);
TOXtesclass <- c(2,1,2,2,3,4,4,2,3,4,3,4,2,1,1,1,4,4,3,2,4,1,1,4,1,3,1,2,1,2,3,2,3,4)

TOXidx <- c(TOXtraidx, TOXtesidx)
TOXclass <- c(TOXtraclass, TOXtesclass)

TOXclass_order <- TOXclass[order(TOXidx)]

# table(data_all$lex, data_all$fx, useNA = "ifany")
# table(data_all$fx_new1, useNA = "ifany")
# table(data_all$buwei, useNA = "ifany")

data_all <- cbind(TOXclass_order, data_all)

name_y_data_all <- "TOXclass_order"
...
for(rep_i in 2:num_rep){
  ...
  for(fold_i in 1:4){
    ...
    ### method 22_2: under sampling (SVM)
