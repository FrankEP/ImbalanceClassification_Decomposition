...
data_all_y <- openxlsx::read.xlsx(paste0(path,"/data_open/", pathname,"/GLIOMA_Y.xlsx"), colNames = FALSE)
data_all_x <- read.csv(paste0(path,"/data_open/", pathname,"/GLIOMA_X.csv"), header = FALSE)

data_all <- cbind(data_all_y, data_all_x)

colnames(data_all) <- c("Class", colnames(data_all_x))

name_y_data_all <- colnames(data_all)[1]
...
for(rep_i in 1:num_rep){
  ...
  for(fold_i in 1:num_Kfold){
    ...
    if(is.element("ATM", names_methods)){
      results_ATM <- Adaptive_threshold_moving(X=data_train_x, Y=data_train_y, NEW_X=data_test_x, num_bootstrap=5, pathname = pathname, fold_i=fold_i, rep_i=rep_i, 
