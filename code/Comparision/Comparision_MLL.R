...
data_all <- openxlsx::read.xlsx(paste0(path,"/data_open/", pathname,"/MLL.xlsx"), colNames = FALSE)

MLLtraidx <- c(9,22,34,35,42,26,59,40,54,17,8,51,60,61,31,55,66,5,50,14,70,30,28,57,47,62,32,
               7,2,53,43,3,44,11,68,25,16,36,24,63,71,15,29,27,49,67,33,23,45,65,4,1,46,52,20,39,19,21);
MLLtesidx <- c(48,6,10,37,38,64,12,69,58,13,41,18,72,56);
MLLtraclass <- c(1,1,2,2,2,2,3,2,3,1,1,3,3,3,2,3,3,1,3,1,3,2,2,3,3,3,2,1,1,3,2,1,2,1,3,2,1,2,1,3,
                 3,1,2,2,3,3,2,1,3,3,1,1,3,3,1,2,1,1);
MLLtesclass <- c(3,1,1,2,2,3,1,3,3,1,2,1,3,3);

MLLidx <- c(MLLtraidx, MLLtesidx)
MLLclass <- c(MLLtraclass, MLLtesclass)

MLLclass_order <- MLLclass[order(MLLidx)]

# table(data_all$lex, data_all$fx, useNA = "ifany")
# table(data_all$fx_new1, useNA = "ifany")
# table(data_all$buwei, useNA = "ifany")

data_all <- cbind(MLLclass_order, data_all)

name_y_data_all <- "MLLclass_order"
