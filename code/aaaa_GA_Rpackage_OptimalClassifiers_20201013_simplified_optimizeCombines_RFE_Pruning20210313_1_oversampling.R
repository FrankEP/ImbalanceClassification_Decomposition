...
Adaptive_threshold_moving <- function(X, Y, NEW_X, num_bootstrap=10, pathname = "directory_temp", fold_i=fold_i, rep_i=rep_i, over_sampling_binary=T,
  ...
  for(bootstrap_i in 1:num_bootstrap){    ## Stack1 Way1: normal for loop
    ...
    auc_model_list_bootstrap <- foreach(combination_i = c(1:ncol(class_combinations))) %DO_1% {       ### Stack2 Way2: dopar loop
      ...
      if(fit_model_binary){
        ...
        if(over_sampling_binary){
                  data_train_y_x_inbag_binary <- caret::upSample(x = data_train_y_x_inbag_binary[, c(names_x_fixed, names_x_NoFixed_sub_temp)],
                                                 y = as.factor(data_train_y_x_inbag_binary$Y))
        colnames(data_train_y_x_inbag_binary) <- c(c(names_x_fixed, names_x_NoFixed_sub_temp), "Y")
        }
...
for (bootstrap_i in 1:num_bootstrap) {
  ...
  weight_combinations[weight_combinations>2.5] <- 2.5
...
# x=GA_results_bootstrap[[bootstrap_i]]@population[6, ]
# # x=GA_results_bootstrap[[bootstrap_i]]@solution
# M=M_coding_temp
...
# )
# 
# GA_results_bootstrap[[bootstrap_i]]@fitness[27]
# 
# a <- GA_results_bootstrap[[bootstrap_i]]@population[1, ]
# 
# fitness_function(x = GA_results_bootstrap[[bootstrap_i]]@population[6,],
#                  M = M_coding_temp,
...
for (bootstrap_i in 1:num_bootstrap) {
  ...
  # if(over_sampling_binary){
  #   GAMMA_initial <- c(table(data_train_y_x_inbag$Y)/table(data_train_y_x_inbag$Y))
  # }
...
for (bootstrap_i in 1:num_bootstrap) {
  ...
  s_order_remain <- c(GA_results_bootstrap[[bootstrap_i]]@fitness>0.85)[s_order]
  ...
  for(s_i in s_order){
    # cat(s_i, "\n")
    if(inherits(try(
      results_s_i <- fitness_function(x = GA_results_bootstrap[[bootstrap_i]]@population[s_i,],
                                                       M = M_coding_temp,
                                                       prediction_matrix_inbag = prediction_train_inbag_AllBinaryClassifiers_bootstrap[[bootstrap_i]],
                                                       N_row_inbag = nrow(prediction_train_inbag_AllBinaryClassifiers_bootstrap[[bootstrap_i]]),
                                                       label_real_inbag = data_train_y_x_inbag$Y,
      ...
      #                                                  N_row_outbag = nrow(prediction_testset_AllBinaryClassifiers_bootstrap[[bootstrap_i]]),
      #                                                  label_real_outbag = NULL)
      class_prediction_inbag_bootstrap <- cbind(class_prediction_inbag_bootstrap, results_s_i$class_prediction_inbag)
      class_prediction_outbag_bootstrap <- cbind(class_prediction_outbag_bootstrap, results_s_i$class_prediction_outbag)
      # break
    }
  }
}
