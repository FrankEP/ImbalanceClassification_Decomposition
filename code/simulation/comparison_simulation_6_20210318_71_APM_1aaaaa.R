...
print("3")
source(paste0(path,'/code/aaaa_GA_Rpackage_OptimalClassifiers_20201013_simplified_optimizeCombines_RFE_Pruning20210313_1_oversampling.R'))
...
pathname <-  "simulation_6_20210318_71_APM_1"
...
names_methods <- c("ATM")
# names_methods <- c("ATM", "over_sampling_RF_OVO", "over_sampling_SVM_OVO", "down_sampling_RF_OVO", "down_sampling_SVM_OVO", "cost_sensitive_RF_OVO", "cost_sensitive_SVM_OVO")
...
names_x_fixed = NULL
subsets_backward <- c(40, 60, 80, 120, 140, 160, 240, 360)
...
for (N_matrix_i in 1:nrow(N_matrix)) {
  ...
  for(rep_i in 1:num_rep){
    ...
    # data_train_list_outlier <- simulation_data_generator(N_classes=floor(N_classes_train*0.1), N_var=N_var, mean_classes=mean_classes, sd_mean_noise=sd_mean_noise, N_clusters=N_clusters, 
    #                                                      sigma_inner_clusters=sigma_inner_clusters, sigma_between_clusters=sigma_between_clusters, 
    #                                                      sd_var=sd_var_outlier, addNoise_correlation=TRUE, sd_noise_correlation=sd_noise_correlation, Seed=1)
    # data_train_outlier <- data_train_list_outlier$data_final
    # 
    # data_train <- c()
    # for (class_i in 1:length(N_classes_train)) {
    #   data_train_class_i <- data_train_temp[data_train_temp$label==class_i, ]
    #   data_train_outlier_class_i <- data_train_outlier[data_train_outlier$label==class_i, ]
    #   
    #   set.seed(class_i)
    #   idx_replace <- base::sample(c(1:nrow(data_train_class_i)), nrow(data_train_outlier_class_i), replace = F)
    #   
    #   data_train_class_i_sub <- data_train_class_i[-c(idx_replace), ]
    #   data_train_class_i <- rbind(data_train_class_i_sub, data_train_outlier_class_i)
    #   
    #   data_train <- rbind(data_train, data_train_class_i)
    # }
    ...
    # library(Publish)
    # var_lx_mean <- paste0("label~", paste0(names_x_data_train, collapse = "+"))
    ...
    # data_test_list_outlier <- simulation_data_generator(N_classes=floor(N_classes_test*0.1), N_var=N_var, mean_classes=mean_classes, sd_mean_noise=sd_mean_noise, N_clusters=N_clusters, 
    #                                                     sigma_inner_clusters=sigma_inner_clusters, sigma_between_clusters=sigma_between_clusters, 
    #                                                     sd_var=sd_var_outlier, addNoise_correlation=TRUE, sd_noise_correlation=sd_noise_correlation, Seed=1)
    # data_test_outlier <- data_test_list_outlier$data_final
    # 
    # 
    # data_test <- c()
    # for (class_i in 1:length(N_classes_test)) {
    #   data_test_class_i <- data_test_temp[data_test_temp$label==class_i, ]
    #   data_test_outlier_class_i <- data_test_outlier[data_test_outlier$label==class_i, ]
    #   
    #   set.seed(class_i)
    #   idx_replace <- base::sample(c(1:nrow(data_test_class_i)), nrow(data_test_outlier_class_i), replace = F)
    #   
    #   data_test_class_i_sub <- data_test_class_i[-c(idx_replace), ]
    #   data_test_class_i <- rbind(data_test_class_i_sub, data_test_outlier_class_i)
    #   
    #   data_test <- rbind(data_test, data_test_class_i)
    # }
    ...
    for(fold_i in 1:num_Kfold){
      ...
      if(is.element("ATM", names_methods)){
        results_ATM <- Adaptive_threshold_moving(X=data_train_x, Y=data_train_y, NEW_X=data_test_x, num_bootstrap=3, pathname = pathname, fold_i=fold_i, rep_i=rep_i, over_sampling_binary=T,
                                                 rfe_select=FALSE, select_backward = FALSE, select_backward_wilcoxon = TRUE, names_x_fixed = NULL,
                                                 subsets_rfe = subsets_backward, parallel_combinations=TRUE, parallel_AdaBoost=FALSE, sub_features=TRUE, ratio_sub_features=0.3, fit_model_binary=T, fit_model_OVA=F,
        ...
        # sub_features=TRUE
        # ratio_sub_features=0.3
        # fit_model_binary=T
        # fit_model_OVA=F
        # rescale_prediction=T
        # plot_scatter=FALSE
        # plot_boxplot_prediction_InbagAndOutbag=F
        # plot_boxplot_prediction_TrainAndTest=F
        # over_sampling_binary=T
