...
Adaptive_threshold_moving <- function(X, Y, NEW_X, num_bootstrap=10, pathname = "directory_temp", fold_i=fold_i, rep_i=rep_i, plot_scatter=FALSE, 
  ...
  ### centriod of each class in training inbag dataset
  ...
  for (class_i in 1:K) {
    ...
    dist_temp <- foreach(i = 1:nrow(prediction_train_inbag_AllBinaryClassifiers_final), .combine = c ) %do% euc.dist(prediction_train_inbag_AllBinaryClassifiers_final[i,], centroid_train_inbag[class_i, 2:ncol(centroid_train_inbag)])
  ...
  for (class_i in 1:K) {
    ...
    dist_temp <- foreach(i = 1:nrow(prediction_train_outbag_AllBinaryClassifiers_final), .combine = c ) %do% euc.dist(prediction_train_outbag_AllBinaryClassifiers_final[i,], centroid_train_outbag[class_i, 2:ncol(centroid_train_inbag)])
  ...
  ### test
  dist_matrix_testset <- c()
  for (class_i in 1:K) {
    
    dist_temp <- foreach(i = 1:nrow(prediction_testset_AllBinaryClassifiers_final), .combine = c ) %do% euc.dist(prediction_testset_AllBinaryClassifiers_final[i,], centroid_train_inbag[class_i, 2:ncol(centroid_train_inbag)])
    
    dist_matrix_testset <- cbind(dist_matrix_testset, dist_temp)
  }
  
  
  class_prediction_testset <- apply(dist_matrix_testset, 1, which.min)
  
  table(data_test_y_numerical, class_prediction_testset)
  
  multiclass.roc(data_test_y_numerical, class_prediction_testset)
  
  F1(data_test_y_numerical, class_prediction_testset)
