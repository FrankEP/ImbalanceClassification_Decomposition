...
Adaptive_threshold_moving <- function(X, Y, NEW_X, num_bootstrap=10, pathname = "directory_temp", fold_i=fold_i, rep_i=rep_i, ECOC_type="ternary complete",
...
#  subsets_rfe <- subsets_rfe * 2
  ...
  for(bootstrap_i in 1:num_bootstrap){    ## Stack1 Way1: normal for loop
    ...
    auc_model_list_bootstrap <- foreach(combination_i = c(1:ncol(class_combinations))) %DO_1% {       ### Stack2 Way2: dopar loop
      ...
      # source(paste0(path, '/code/1_function_ECOC_code_matrix_generator.R'))
      # M_coding <- designecoc(K=K, dname = ECOC_type)
      
      ## extract binary dataset
      # dichotomy_i <- M_coding[, combination_i]
      # class_1 <- c(1:K)[dichotomy_i==1]
      # class_2 <- c(1:K)[dichotomy_i==-1]
      # 
      # class_others <- c(1:K)[dichotomy_i==0]
      ...
      data_train_y_x_inbag_class1 <- data_train_y_x_inbag[is.element(data_train_y_x_inbag$Y, class_1), ]
      data_train_y_x_inbag_class2 <- data_train_y_x_inbag[is.element(data_train_y_x_inbag$Y, class_2), ]
      ...
      if(fit_model_binary){
        ...
        data_train_y_x_inbag_binary <- rbind(data_train_y_x_inbag_class1, data_train_y_x_inbag_class2)
        data_train_y_x_inbag_binary$Y_binary_numerical <- ifelse(is.element(data_train_y_x_inbag_binary$Y, class_1), 1, -1)
        data_train_y_x_inbag_binary$Y_binary_factor <- as.factor(data_train_y_x_inbag_binary$Y_binary_numerical)
      }
      ...
      data_train_y_x_outbag_class1 <- data_train_y_x_outbag[is.element(data_train_y_x_outbag$Y, class_1), ]
      data_train_y_x_outbag_class2 <- data_train_y_x_outbag[is.element(data_train_y_x_outbag$Y, class_2), ]
      ...
      if(fit_model_binary){
        ...
        data_train_y_x_outbag_binary <- rbind(data_train_y_x_outbag_class1, data_train_y_x_outbag_class2)
        data_train_y_x_outbag_binary$Y_binary_numerical <- ifelse(is.element(data_train_y_x_outbag_binary$Y, class_1), 1, -1)
        data_train_y_x_outbag_binary$Y_binary_factor <- as.factor(data_train_y_x_outbag_binary$Y_binary_numerical)
      }
...
for (bootstrap_i in 1:num_bootstrap) {
  ...
  for (combination_i in c(1:ncol(M_coding))) {
    cat("combination_i ", combination_i, "\n")
    ...
    for (model_i in model_names) {
      ...
      ###  identify the best parameter of the current model under current combination and current bootstrap
      ...
      cluster_criteria_parameters_temp <- cluster_criteria_parameters_outbag
      ...
      if(max(auc_clusterCriteria_parameters_grid$auc_binary_temp_outbag) > 0.85){
        auc_include_boolean <- auc_clusterCriteria_parameters_grid$auc_binary_temp_outbag>0.85
      } else {
        auc_include_boolean <- auc_clusterCriteria_parameters_grid$auc_binary_temp_outbag==max(auc_clusterCriteria_parameters_grid$auc_binary_temp_outbag)
      }
      ...
      auc_best_parameter_temp <- auc_clusterCriteria_parameters_grid$auc_binary_temp_outbag[idx_best_parameter_temp]
...
for (bootstrap_i in 1:num_bootstrap) {
  ...
  idx_models[[bootstrap_i]] <- apply(clusterCtriteria_best_parameter_model_combination, 2, which.min)
  idx_combinations[[bootstrap_i]] <- c(1:ncol(M_coding))
  ...
  #### weights of base classifiers (method1)
  ...
  # weight_combinations[weight_combinations==Inf] <- 2
  #### 
  
  #### weights of base classifiers (method2)
  auc_models <- c()
  for (i in 1:length(idx_models[[bootstrap_i]])) {
    auc_models[i] <- auc_best_parameter_model_combination[idx_models[[bootstrap_i]][i], i]
  }
  
  idx_combination_remian[[bootstrap_i]] <- auc_models>0.0
  
  weight_combinations <- 0.5*log(auc_models/(1-auc_models))
...
for (bootstrap_i in 1:num_bootstrap) {
  ...
  for (combination_i in seq_len(length(idx_combinations[[bootstrap_i]]))) {
    ...
    if(plot_boxplot_prediction_InbagAndOutbag){
      ...
      for (i in 2:ncol(prediction_train_inbag_AllBinaryClassifiers_boxplot)) {
        ...
        ggsave(filename = paste0(path,"/results/", pathname,"/boxplot_prediction/", "rep_i_", rep_i, "_fold_i_", fold_i, "_bootstrap_", bootstrap_i, "_inbag_", x_i, ".png"), boxplot_age_45, height = 10, width = 10)
      }
    }
  ...
  ##########  outbag  #########
    prediction_train_outbag_AllBinaryClassifiers_list <- list()
    num_outbag <- 10
    for (outbag_i in 1:num_outbag) {
      
      ### add noise to data_train_y_x_outbag
      noise_matrix <- matrix(rnorm(n = c(nrow(data_train_y_x_outbag))*c(ncol(data_train_y_x_outbag)-1), mean = 0, sd = 1), nrow = nrow(data_train_y_x_outbag))
      data_train_y_x_outbag_i <- cbind(data_train_y_x_outbag$Y, data_train_y_x_outbag[, c(names_x_fixed, names_x_NoFixed_sub_temp)]+ noise_matrix)
      colnames(data_train_y_x_outbag_i) <- c("Y", names_x_fixed, names_x_NoFixed_sub_temp)
      ...
      for (combination_i in seq_len(length(idx_combinations[[bootstrap_i]]))) {
        ...
        if(is.element(model_names[idx_best_model], c("RandomForest"))){
          ...
          ### binary
          if(fit_model_binary){
            prediction_outbag <- predict(model_fit, data=data_train_y_x_outbag_i, probability = TRUE)$predictions
          ...
          if(fit_model_OVA){
            ### prediction OVA1
            prediction_AllClass_outbag_OVA1_temp <- predict(model_fit_OVA1, data = data_train_y_x_outbag_i)$predictions
            prediction_AllClass_outbag_OVA1_temp <- prediction_AllClass_outbag_OVA1_temp[, 2]
            ratio_outbag_OVA1_temp <- 1/(max(prediction_AllClass_outbag_OVA1_temp) - min(prediction_AllClass_outbag_OVA1_temp))
            prediction_AllClass_outbag_OVA1_temp <- (prediction_AllClass_outbag_OVA1_temp - min(prediction_AllClass_outbag_OVA1_temp)) * ratio_outbag_OVA1_temp
            prediction_AllClass_outbag_OVA1_temp <- -1 + prediction_AllClass_outbag_OVA1_temp*2
            ...
            ### prediction OVA2
            prediction_AllClass_outbag_OVA2_temp <- predict(model_fit_OVA2, data = data_train_y_x_outbag_i)$predictions
            prediction_AllClass_outbag_OVA2_temp <- prediction_AllClass_outbag_OVA2_temp[, 2]
            ratio_outbag_OVA2_temp <- 1/(max(prediction_AllClass_outbag_OVA2_temp) - min(prediction_AllClass_outbag_OVA2_temp))
            prediction_AllClass_outbag_OVA2_temp <- (prediction_AllClass_outbag_OVA2_temp - min(prediction_AllClass_outbag_OVA2_temp)) * ratio_outbag_OVA2_temp
            prediction_AllClass_outbag_OVA2_temp <- -1 + prediction_AllClass_outbag_OVA2_temp*2
            ...
            if(fit_model_binary){
              # all classes outbag
              prediction_outbag <- predict(model_fit, newdata = data_train_y_x_outbag_i, probability = TRUE)
            ...
            if(fit_model_OVA){
              # all classes outbag
              ### prediction OVA1
              prediction_AllClass_outbag_OVA1_temp <- predict(model_fit_OVA1, newdata = data_train_y_x_outbag_i, probability = TRUE)
              prediction_AllClass_outbag_OVA1_temp <- attr(prediction_AllClass_outbag_OVA1_temp, "probabilities")
              prediction_AllClass_outbag_OVA1_temp <- prediction_AllClass_outbag_OVA1_temp[, "1"]
              ratio_outbag_OVA1_temp <- 1/(max(prediction_AllClass_outbag_OVA1_temp) - min(prediction_AllClass_outbag_OVA1_temp))
              prediction_AllClass_outbag_OVA1_temp <- (prediction_AllClass_outbag_OVA1_temp - min(prediction_AllClass_outbag_OVA1_temp)) * ratio_outbag_OVA1_temp
              ...
              ### prediction OVA2
              prediction_AllClass_outbag_OVA2_temp <- predict(model_fit_OVA2, newdata = data_train_y_x_outbag_i, probability = TRUE)
              prediction_AllClass_outbag_OVA2_temp <- attr(prediction_AllClass_outbag_OVA2_temp, "probabilities")
              prediction_AllClass_outbag_OVA2_temp <- prediction_AllClass_outbag_OVA2_temp[, "1"]
              ratio_outbag_OVA2_temp <- 1/(max(prediction_AllClass_outbag_OVA2_temp) - min(prediction_AllClass_outbag_OVA2_temp))
              prediction_AllClass_outbag_OVA2_temp <- (prediction_AllClass_outbag_OVA2_temp - min(prediction_AllClass_outbag_OVA2_temp)) * ratio_outbag_OVA2_temp
          ...
          prediction_outbag <- predict(model_fit, newdata=data_train_y_x_outbag_i)
        }
      ...
      # plot_boxplot_prediction_InbagAndOutbag <- TRUE
      if(plot_boxplot_prediction_InbagAndOutbag){
        prediction_train_outbag_AllBinaryClassifiers_boxplot <- cbind(data_train_y_x_outbag_i$Y, prediction_train_outbag_AllBinaryClassifiers)
        ...
        for (i in 2:ncol(prediction_train_outbag_AllBinaryClassifiers_boxplot)) {
          ...
          ggsave(filename = paste0(path,"/results/", pathname,"/boxplot_prediction/", "rep_i_", rep_i, "_fold_i_", fold_i, "_bootstrap_", bootstrap_i, "_outbag_i_",  outbag_i, "_outbag_", x_i, ".jpeg"), boxplot_age_45, height = 10, width = 10)
        }
      }
      ...
      ### !!!!! keep the predictions of remained classifiers
      prediction_train_outbag_AllBinaryClassifiers_list[[outbag_i]] <- prediction_train_outbag_AllBinaryClassifiers[, idx_combination_remian[[bootstrap_i]]]
      
    }
    
    prediction_train_outbag_AllBinaryClassifiers_bootstrap[[bootstrap_i]] <- prediction_train_outbag_AllBinaryClassifiers_list
...
fitness_function <- function(x, M, prediction_matrix_inbag, N_row_inbag, label_real_inbag, num_combination, prediction_matrix_outbag, N_row_outbag, label_real_outbag){
  ...
  if(is.null(dim(prediction_matrix_outbag))){
    ## part 2: adjust the output of each binary classifier on outbag dataset using respective adjust_value 
    num_outbag <- length(prediction_matrix_outbag)
    F1_outbag_noise <- c()
    for (outbag_i in 1:num_outbag) {
      
      prediction_matrix_outbag_noise <- prediction_matrix_outbag[[outbag_i]]
      
      ### part 2.2: outbag
      adjust_values_matrix_outbag <- t(replicate(nrow(prediction_matrix_outbag_noise), adjust_values))
      
      prediction_matrix_outbag_noise <- as.matrix(prediction_matrix_outbag_noise) + adjust_values_matrix_outbag
      ...
      for (i in 1:N_row_outbag) {
        # cat(i, "  ", "\n")
        loss_i <- -M%*%as.numeric(prediction_matrix_outbag_noise[i, ])
      ...
      Fi <- 2*PRECISION*RECALL/(PRECISION+RECALL)
      Fi <- ifelse(is.na(Fi), 0, Fi)
      F1_outbag_noise_i <- mean(Fi)
      F1_outbag_noise <- c(F1_outbag_noise, F1_outbag_noise_i)
      confusion_matrix_outbag <- NA
    }
    (F1_outbag <- min(F1_outbag_noise))
  } else {
    ### part 2.2: outbag
...
# x = GA_results_bootstrap[[bootstrap_i]]@population[s_i,]
# M = M_coding_temp
# prediction_matrix_inbag = prediction_train_inbag_AllBinaryClassifiers_bootstrap[[bootstrap_i]]
# N_row_inbag = nrow(prediction_train_inbag_AllBinaryClassifiers_bootstrap[[bootstrap_i]])
...
for (bootstrap_i in 1:num_bootstrap) {
  ...
  prediction_train_outbag_AllBinaryClassifiers_list <- prediction_train_outbag_AllBinaryClassifiers_bootstrap[[bootstrap_i]]
  ...
  while (inherits( try(
    aa <- fitness_function(x = x,
                           ...
                           label_real_inbag = data_train_y_x_inbag$Y,
                           num_combination = ncol(M_coding_temp),
                           prediction_matrix_outbag = prediction_train_outbag_AllBinaryClassifiers_list,
                           N_row_outbag = nrow(data_train_y_x_outbag),
                           label_real_outbag = data_train_y_x_outbag$Y)
...
for (bootstrap_i in 1:num_bootstrap) {
  ...
  if(plot_boxplot_prediction_TrainAndTest){
    ...
    for (i in 2:ncol(prediction_trainset_AllBinaryClassifiers_boxplot)) {
      ...
      ggsave(filename = paste0(path,"/results/", pathname,"/boxplot_prediction/", "rep_i_", rep_i, "_fold_i_", fold_i, "_bootstrap_", bootstrap_i, "_trainset_", x_i, ".jpeg"), boxplot_age_45, height = 10, width = 10)
    }
    ...
    for (i in 2:ncol(prediction_testset_AllBinaryClassifiers_boxplot)) {
      ...
      ggsave(filename = paste0(path,"/results/", pathname,"/boxplot_prediction/", "rep_i_", rep_i, "_fold_i_", fold_i, "_bootstrap_", bootstrap_i, "_testset_", x_i, ".jpeg"), boxplot_age_45, height = 10, width = 10)
    }
  }
...
for (bootstrap_i in 1:num_bootstrap) {
  ...
  s_order <- order(GA_results_bootstrap[[bootstrap_i]]@fitness, decreasing = TRUE)
  s_order_remain <- c(GA_results_bootstrap[[bootstrap_i]]@fitness>0.9)[s_order]
  if(!is.na(any(s_order_remain))){
    s_order <- na.omit(s_order[s_order_remain])
  } else {
    s_order <- s_order[1]
  }
  ...
  for(s_i in s_order){
    ...
    if(inherits(try(
      results_final[[bootstrap_i]] <- fitness_function(x = GA_results_bootstrap[[bootstrap_i]]@population[s_i,],
                                                       ...
                                                       label_real_inbag = data_train_y_x_inbag$Y,
                                                       num_combination = ncol(M_coding_temp),
                                                       prediction_matrix_outbag = as.data.frame(prediction_testset_AllBinaryClassifiers_bootstrap[[bootstrap_i]]),
                                                       N_row_outbag = nrow(prediction_testset_AllBinaryClassifiers_bootstrap[[bootstrap_i]]),
                                                       label_real_outbag = NULL)
    ), "try-error")){
      next
